<template>
  <div :class="`box-background p-8 rounded-lg w-80 ${cardBorder ? 'border-2 border-purple-500' : ''}`">
    ...
    <div class="flex justify-center">
      <IconButton :iconClass="iconClass" class="mb-4" :href="buttonLink" :primary="false">{{ buttonText }}</IconButton>
    </div>
    <ul class="text-left light:text-light-primary dark:text-dark-primary space-y-2">
      ...
      <li :key="totalUploadCap">
        <i :class="`fas fa-check-circle font-bold ${titleClass}`"></i>
        Upload up to {{ prettyBytes(totalUploadCap) }} of files in total
      </li>
